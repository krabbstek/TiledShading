...
Cube::Cube(const vec3& position /*= { 0.0f, 0.0f, 0.0f }*/, const vec3& color /*= { 1.0f, 1.0f, 1.0f }*/)
	: position(position)
{
	for (int i = 0; i < 24; i++)
	{
		cubeVertices[i * 9 + 6] = color.r;
		cubeVertices[i * 9 + 7] = color.g;
		cubeVertices[i * 9 + 8] = color.b;
	}
...
void Cube::Render(Renderer& renderer, GLShader& shader)
{
	shader.Bind();
